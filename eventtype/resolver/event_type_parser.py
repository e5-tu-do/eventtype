...
def get_g_info(g_flag):
    ...
    g_dict[1] = {
        "meaning": "events containing a b quark, extracted from a minimum bias sample", type: "info"}
    g_dict[2] = {
        "meaning": "events containing a c quark, extracted from a mimimum bias sample", type: "info"}
    g_dict[3] = {
        "meaning": "events without b and c quark, extracted from a minimum bias sample", type: "info"}
    g_dict[4] = {
        "meaning": "non-flavour physics with special generator settings", type: "info"}
    g_dict[5] = {"meaning": "particle guns", type: "info"}
    g_dict[6] = {
        "meaning": "machine induced background (MIB) events", type: "info"}
...
def get_s_info_dict(g_flag):
    ...
    if g_flag == 1:
        s_dict[0] = {
            "meaning": "a bhadron, amongst a list of more than one specified species, is required", "type": "info"}
        ...
        s_dict[6] = {
            "meaning": "a specific b-baryon, other than $\Lambda_b$, is required the extra flag specifies the type of baryon", "type": "info"}
        s_dict[7] = {
            "meaning": "a b-hadron resonance is required, the extra flag specifies the type of resonance", "type": "info"}
        s_dict[8] = {
            "meaning": "a bb̄-meson  is required, the extra flag specifies the type of resonance", "type": "info"}
        s_dict[9] = {
            "meaning": "two or more b-hadrons, amongst a list of specified combinations, are required", "type": "info"}

    elif g_flag == 2:
        s_dict[0] = {
            "meaning": "a c-hadron, amongst a list of more than one specified species, is required", "type": "info"}
        ...
        s_dict[7] = {
            "meaning": "a c-hadron resonance is required, the extra flag specifies the type of resonance", "type": "info"}
        s_dict[8] = {
            "meaning": "a (cc̄-meson other than $J/\psi$) is required, the extra flag specifies the type of resonance", "type": "info"}
        s_dict[9] = {
            "meaning": "two or more c-hadrons, amongst a list of specified combinations, are required", "type": "info"}

    elif g_flag == 3:
        s_dict[0] = {"meaning": "no requirement", "type": "info"}
        s_dict[1] = {
            "meaning": "a $\\tau^+$ + or $\\tau^-$ is required. (See LHCb Note 2009-001 for more details)", "type": "info"}
        s_dict[2] = {
            "meaning": "a $\Sigma^+$ or $\Sigma^-$ is required", "type": "info"}
        s_dict[3] = {
            "meaning": "a $\Lambda$ or $\\bar{\Lambda}$ is required", "type": "info"}
        s_dict[4] = {"meaning": "a $K_S^0$ is required", "type": "info"}
        s_dict[5] = {
            "meaning": "an $\Omega^-$ or $\Omega^+$ is required", "type": "info"}
        ...
        s_dict[3] = {
            "meaning": "a Hidden Valley through Higgs is required", "type": "info"}
        s_dict[4] = {
            "meaning": "a Hidden Valley with multi v-pion is required", "type": "info"}
        ...
        s_dict[0] = {
            "meaning": "no particular type of particle generated (i.e. all available types)", "type": "info"}
...
def get_d_info_dict(g_flag, c_flag, s_flag):
    ...
    if g_flag in [1, 2, 3, 4]:
        d_dict[0] = {
            "meaning": "No decay is forced (fully inclusive mode)", "type": "info"}
        d_dict[1] = {
            "meaning": "The selected particle(s) is(are) forced to decay to a unique exclusive final state, with a unique intermediate state (if any)", "type": "info"}
        d_dict[2] = {
            "meaning": "The selected particle(s) is(are) forced to decay to a unique exclusive final state, with multiple intermediate states", "type": "info"}
        d_dict[3] = {
            "meaning": "The selected particle(s) is(are) forced to decay to one of several exclusive final states (explicitely specified in a list) with identical topology", "type": "info"}
        d_dict[4] = {
            "meaning": "The selected particle(s) is(are) forced to decay to one of several exclusive final states (explicitely specified in a list) with different topologies", "type": "info"}
        d_dict[5] = {
            "meaning": "The selected particle(s) is(are) forced to decay to a unique exclusive final state, with a unique intermediate state (if any), but the final state(s) contain(s) at least one particle which cannot be measured (neutrinos)", "type": "info"}
        d_dict[6] = {
            "meaning": "The selected particle(s) is(are) forced to decay to a unique exclusive final state, with multiple intermediate states, but the final state(s) contain(s) at least one particle which cannot be measured (neutrinos)", "type": "info"}
        d_dict[7] = {
            "meaning": "The selected particle(s) is(are) forced to decay to one of several exclusive final states (explicitely specified in a list) with identical topology, but the final state(s) contain(s) at least one particle which cannot be measured (neutrinos)", "type": "info"}
        d_dict[8] = {
            "meaning": "The selected particle(s) is(are) forced to decay to one of several exclusive final states (explicitely specified in a list) with different topologies, but the final state(s) contain(s) at least one particle which cannot be measured (neutrinos)", "type": "info"}
        d_dict[9] = {
            "meaning": "The selected particle(s) is(are) forced to a decay chain containing some inclusive decays (semi-inclusive mode), resulting in a number of different final states which are not explicitely specified", "type": "info"}
        ...
        d_dict[0] = {
            "meaning": "gaussian p-Gas vertex distribution in x,y, uniform in z", "type": "info"}
        d_dict[1] = {
            "meaning": "uniform p-Gas vertex distribution in x,y and z", "type": "info"}
        d_dict[2] = {"meaning": "reserve", "type": "error"}
        d_dict[3] = {"meaning": "reserve", "type": "error"}
        d_dict[4] = {"meaning": "reserve", "type": "error"}
        d_dict[5] = {"meaning": "reserve", "type": "error"}
        ...
        d_dict[0] = {
            "meaning": "staged shield, loss on TCTV.4L8.B1 & TCTH.L8.B1", "type": "info"}
        d_dict[1] = {
            "meaning": "staged shield, loss on TCTV.4L8.B1", "type": "info"}
        d_dict[2] = {
            "meaning": "staged shield, loss on TCTH.L8.B1", "type": "info"}
        d_dict[3] = {
            "meaning": "design shield, loss on TCTV.4L8.B1 & TCTH.L8.B1", "type": "info"}
        d_dict[4] = {
            "meaning": "design shield, loss on TCTV.4L8.B1", "type": "info"}
        d_dict[5] = {
            "meaning": "design shield, loss on TCTH.L8.B1", "type": "info"}
        d_dict[6] = {
            "meaning": "no shield, loss on TCTV.4L8.B1 & TCTH.L8.B1", "type": "info"}
        d_dict[7] = {
            "meaning": "no shield, loss on TCTH.L8.B1", "type": "info"}
        d_dict[8] = {
            "meaning": "no shield, loss on TCTH.L8.B1", "type": "info"}
        d_dict[9] = {"meaning": "reserve", "type": "error"}
    return d_dict
...
def get_c_info_dict(g_flag):
    ...
    if g_flag in [1, 2, 3, 4]:
        ...
        c_dict[0] = {
            "meaning": "no $c\\bar{c}$ meson, no open-charm hadron, no muon no electron", "type": "info"}
        c_dict[1] = {
            "meaning": "no $c\\bar{c}$ meson, no open-charm hadron, at least 1 muon", "type": "info"}
        c_dict[2] = {
            "meaning": "no $c\\bar{c}$ meson, no open-charm hadron, no muon , electron : at least 1", "type": "info"}

        c_dict[3] = {
            "meaning": "at least 1 $c\\bar{c}$ meson, no open-charm hadron, no muon , no electron", "type": "info"}
        c_dict[4] = {
            "meaning": "at least 1 $c\\bar{c}$ meson, no open-charm hadron, at least 1 muon", "type": "info"}
        c_dict[5] = {
            "meaning": "at least 1 $c\\bar{c}$ meson, no open-charm hadron, no muon , at least 1 electron ", "type": "info"}

        c_dict[6] = {
            "meaning": "one open-charm hadron, no muon, no electron", "type": "info"}
        c_dict[7] = {
            "meaning": "at least one open-charm hadron, at least 1 muon", "type": "info"}
        c_dict[8] = {
            "meaning": "at least one open-charm hadron, no muon, at least 1 electron", "type": "info"}

        c_dict[9] = {
            "meaning": "at least 2 open-charm hadrons, no muon, no electron", "type": "info"}

    elif g_flag == 5:
        c_dict[0] = {
            "meaning": "fixed momentum (specified in other flags) in calo acceptance", "type": "info"}
        c_dict[1] = {
            "meaning": "fixed momentum (specified in other flags) in trackers acceptance", "type": "info"}
        c_dict[2] = {"meaning": "reserve", "type": "error"}
        c_dict[3] = {
            "meaning": "momentum range in calo acceptance", "type": "info"}
        c_dict[4] = {
            "meaning": "momentum range in trackers acceptance", "type": "info"}
        c_dict[5] = {
            "meaning": "flat $\eta$ (range specified in other flags) or flat x-y, flag set to 5", "type": "info"}
        c_dict[6] = {
            "meaning": "flat $\eta$ (range specified in other flags) or flat x-y, flag set to 6", "type": "info"}
        c_dict[7] = {
            "meaning": "flat $\eta$ (range specified in other flags) or flat x-y, flag set to 7", "type": "info"}
        c_dict[8] = {
            "meaning": "flat $\eta$ (range specified in other flags) or flat x-y, flag set to 8", "type": "info"}
        c_dict[9] = {
            "meaning": "flat $\eta$ (range specified in other flags) or flat x-y, flag set to 9", "type": "info"}

    elif g_flag == 6:
        c_dict[0] = {"meaning": "beam gas in VeLo", "type": "info"}
        c_dict[1] = {
            "meaning": "beam gas in VeLo and beam pipe", "type": "info"}
        c_dict[2] = {
            "meaning": "beam gas in IR8 Long Straight Section (LSS)", "type": "info"}
...
def get_t_info(t_flag, g_flag, c_flag):
    ...
    if g_flag == 6:
        ...
        t_dict[1] = {
            "meaning": "beam 1 (traveling from VeLo to Muon system)", "type": "info"}
        t_dict[2] = {
            "meaning": "beam 2 (traveling from Muon system to VeLo)", "type": "info"}
...
def get_n_info(n_flag, g_flag, c_flag, s_flag):
    ...
    if g_flag == 6 and c_flag in [0, 1]:
        ...
        if binary[-1] == "1":
            meaning += [
                "at least one $K_S^0 \\to \pi^+ \pi^-$, $\Lambda \\to p \pi^-$ or  $\Lambda \\to \\bar{p} \pi^+$"]
        if binary[-2] == "1":
...
def get_x_info_dict(g_flag, s_flag):
    ...
    if g_flag == 1 and s_flag == 6:
        ...
        x_dict[4] = {
            "meaning": "a Ξ 0 b or Ξ̄ 0 b is required", "type": "info"}
        ...
        x_dict[2] = {
            "meaning": "a B s ∗0 or B̄ s ∗0 is required", "type": "info"}
        x_dict[3] = {
            "meaning": "a B 1 0 (L) or B̄ 1 0 (L) is required", "type": "info"}
        x_dict[4] = {
            "meaning": "a B 1 + (L) or B 1 − (L) is required", "type": "info"}
        x_dict[5] = {
            "meaning": "a B s1 (L) or B̄ s1 (L) is required", "type": "info"}
        x_dict[6] = {
            "meaning": "a B 2 (L) or B̄ 2 ∗0 (L) is required", "type": "info"}
        x_dict[7] = {
            "meaning": "a B 2 ∗+ (L) or B 2 ∗− (L) is required", "type": "info"}
        ...
        x_dict[0] = {
            "meaning": "a Σ 0 c or Σ̄ 0 c is required", "type": "info"}
        ...
        x_dict[4] = {
            "meaning": "a Σ ∗++ or Σ̄ ∗−− is required", "type": "info"}
        x_dict[5] = {
            "meaning": "a Ξ cc or Ξ̄ − cc is required", "type": "info"}
        x_dict[6] = {
            "meaning": "a Ξ ++ cc or Ξ̄ cc is required", "type": "info"}
        ...
        x_dict[2] = {
            "meaning": "a D s ∗+ or D s ∗− is required", "type": "info"}
        x_dict[3] = {
            "meaning": "a D s2(2536) + or D s2(2536) − is required", "type": "info"}
        x_dict[4] = {
            "meaning": "a D 2 (2460) or D̄ 2 (2460) − is required", "type": "info"}
        x_dict[5] = {
            "meaning": "a D 2 ∗ (2460) + or D 2 ∗ (2460) − is required", "type": "info"}
        x_dict[6] = {
            "meaning": "a D s1 (2460) + or D s1 (2460) − is required", "type": "info"}
